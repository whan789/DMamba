...
class MLPBlock(nn.Module):
    def __init__(self, d_model, enc_in): # enc_in은 이제 사용되지 않지만 호환성을 위해 유지
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)

        # 채널(d_model) 방향으로 MLP 적용
        self.ff1 = nn.Sequential(
            nn.Linear(d_model, d_model * 4), # MLP 확장
            nn.GELU(),
            nn.Dropout(0.1),
        ...
        # 시퀀스(L) 방향으로 MLP 적용
        # 이 부분은 입력 시퀀스 길이에 따라 Linear 레이어 크기가 바뀌므로,
        # 채널 방향 MLP만 사용하는 것이 더 일반적이고 효과적입니다.
        # 여기서는 ff1 (채널 믹싱)만 사용하겠습니다.
...
class Model(nn.Module):
    def __init__(self, configs, mamba_class, dropout=0.1):
        ...
        # --- Graph Layer 대신 MLP Decoder(Head) 사용 ---
        self.mlp_decoder = nn.ModuleList([
            MLPBlock(configs.d_model, configs.enc_in)
            for _ in range(2) # 2개의 MLP 블록을 쌓는 예시
        ])
